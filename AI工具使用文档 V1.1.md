...
- 本文使用包括 Qwen 3.5、GPT-5 mini 在内的大语言模型进行辅助写作（包括信息搜集、代码示例等）与排版。所有内容已经经过人工校对，并努力遵循一些通用的 AI 辅助写作原则
...
- [**AI工具使用文档 V1.1**](#ai工具使用文档-v11)
  ...
  - [**4 提示工程**](#4-提示工程)
    ...
    - [4.2 CO-STAR框架](#42-co-star框架)
    - [4.3 提示词使用技巧](#43-提示词使用技巧)
  - [**5 Agent、MCP、 Skill 与 Harness**](#5-agentmcp-skill-与-harness)
    - [5.1 Agent（智能体）](#51-agent智能体)
      - [5.1.1 如何理解Agent？](#511-如何理解agent)
      - [5.1.2 Agent 示例](#512-agent-示例)
    - [5.2 MCP（模型上下文协议）](#52-mcp模型上下文协议)
      - [5.2.1 什么是MCP？](#521-什么是mcp)
      - [5.2.2 为什么需要 MCP？](#522-为什么需要-mcp)
      - [5.2.3 一个本地部署 MCP 服务器的示例](#523-一个本地部署-mcp-服务器的示例)
      - [5.2.4 补充](#524-补充)
    - [5.3 Agent Skill（智能体技能）](#53-agent-skill智能体技能)
      - [5.3.1 什么是Agent Skills？](#531-什么是agent-skills)
      - [5.3.2 Skill 项目的一个最小结构](#532-skill-项目的一个最小结构)
      - [5.3.3 SKILL.md 字段示例](#533-skillmd-字段示例)
      - [5.3.4 设计原则](#534-设计原则)
    - [5.4 Harness （约束框架）](#54-harness-约束框架)
      - [5.4.1 什么是Harness？](#541-什么是harness)
      - [5.4.2 为什么需要Harness？](#542-为什么需要harness)
      - [5.4.3 架构示意](#543-架构示意)
      - [5.4.4 设计原则](#544-设计原则)
      - [5.4.5 使用时机](#545-使用时机)
      - [5.4.6 Claude Code 意外泄露源码](#546-claude-code-意外泄露源码)
    - [5.5 四者协同](#55-四者协同)
      - [5.5.1 什么在何处负责](#551-什么在何处负责)
      - [5.5.2 典型数据与调用流](#552-典型数据与调用流)
      - [5.5.3 常见协同模式与场景](#553-常见协同模式与场景)
      - [5.5.4 设计原则与最佳实践](#554-设计原则与最佳实践)
      - [5.5.5 简要示例](#555-简要示例)
      - [5.5.6 Q\&A](#556-qa)
      - [5.5.7 建议](#557-建议)
...
### 理解以下概念（至少前面几个）至关重要，无论你打算还是已经使用了AI工具
...
1. **引用格式优化**：利用ChatGPT自动生成和调整引用格式，从而节省时间，使学生能更专注于内容的深度构建。
2. **快速了解新领域**：ChatGPT助力学生迅速掌握新领域的基础知识，为深入探究奠定基础。
3. **资源推荐**：ChatGPT可推荐相关学者、资源和关键词，引导学生研究方向，但需查阅原始文献以保障学术严谨性。
...
10. **写作持续改进**：ChatGPT提供即时反馈，助力学生不断优化写作质量。
11. **语音模式辅助阅读**：在阅读过程中，利用高级语音模式即时解答问题，提升学生对复杂内容的理解和吸收。
12. **技能提升和应用**：帮助学生锻炼分析和写作技能，通过反复练习提升逻辑性和表达能力。

**重要提醒**：学生在使用ChatGPT辅助写作时，必须遵守学术规范，确保AI使用的合规性，规范引用并明确说明自身如何使用ChatGPT。善用AI工具，使其成为学习中的得力助手，而非替代独立思考的捷径。

**链接**：<https://openai.com/chatgpt/use-cases/student-writing-guide/>

### 2.2 西南交通大学 —— 《生成式AI写作指南V1.0》

《生成式AI写作指南V1.0》由西南交通大学多学科专家团队撰写，于2024年10月发布。该指南分三部分，旨在为高校及社会运用生成式AI写作提供参考和指导。以下是内容概览：

1. **基础理论**：阐述生成式AI写作的特征、方法及常用工具，提出这是一种“人机协同的创作新模式”
2. **应用场景**：详细介绍了在私人、公共、专业及教学性设计等不同场景中的AI写作应用与实践，并附案例
3. **风险和规范**：分析了生成式AI写作的潜在风险（如内容准确性、数据偏见、削弱批判性思考等），并明确了AI工具的定位，提出了避免学术不端、正确标注AI成果等规范建议
...
| 方向               | 内涵                                                                                       | 应用                                                                                                                                                                                                                                                           |
| :----------------- | :----------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **评估体系化**     | 对AI输出的反思不再仅依赖用户个人判断，而是发展出一套**系统化的评估框架、基准和自动化工具** | • **红队测试与评估基准**：研究人员和开发者使用标准化测试集（如MMLU、GPQA）和对抗性测试（红队攻击）来系统性评估模型的准确性、安全性、偏见等<br>• **AI辅助评估**：使用另一个AI模型（评判器）来评估生成内容的质量、与指令的对齐程度，实现大规模、自动化的初步反思 |
| **对齐与安全内化** | 反思的目标，从追求“准确答案”提高到“AI对齐”：**安全、合规、符合公序良俗**                   | • **安全层与内容过滤**：在模型响应生成前后，自动进行安全检查，过滤有害、偏见或不合规内容<br>• **宪法AI与RLHF**：通过人类反馈强化学习等技术，将人类的价值观和判断标准“内化”到模型中，使模型在生成时即进行内在的“反思”与对齐                                     |
| **可解释性与溯源** | 反思不仅关注“答案是什么”，更关注“答案为什么是这样”，要求AI提供推理过程和来源依据           | • **思维链**：鼓励AI展示其推理步骤，使用户能跟踪并反思其逻辑<br>• **溯源与引用**：对于基于检索的回答，要求AI明确标注信息出处，方便用户回溯验证，这是“信任但要验证”原则的技术实现                                                                               |
...
**提示词**在AI领域中扮演着至关重要的角色，它指的是在AI模型执行任务时，为引导模型生成特定输出而提供的一段文字或数据，是用户与AI模型进行交互的一种方式。通过精心设计合理的提示词，用户可以更精准地控制AI模型的输出结果。简而言之，提示词就是用户与AI对话的文本，本质上是对AI提出的问题或指令。

**提示工程**（Prompt Engineering）则是指通过优化和设计提示词，以最大化AI输出的效果，从而更高效地利用大型语言模型（LLM）处理各类应用场景和研究领域的问题。这不仅仅是简单的提问，而是需要考虑如何分解问题、设置合适的上下文以及提供必要的细节背景等。其核心目标是找到最优的提示词，使AI能够最大限度地理解任务，并输出符合预期的高质量内容。换言之，学会设计出"最聪明的提问"，以便AI能够聪明地回应。

随着通用大模型的日益成熟，有些时候不再需要严格遵循模板来构思提示词，AI工具已能够深入理解用户需求，并进行自主解析与细化。然而，深入理解提示词的核心逻辑依然至关重要。“提示词工程师”这个职业的兴起消亡提醒我们，“提示工程”已经从一种职业能力转变为专业素养 —— 就像外语之于专业/职业一样，并最终会向“通用素养”这个方向发展。
...
| 模块  | 英文全称                    | 核心定义（"AI 需要知道什么"）                                                                            | 示例（产品推广文案场景）                                                                                                                                                                                                       | 常见误区                                                                       |
| :---- | :-------------------------- | :------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------- |
...
- **提供富含细节的背景信息**  
    例：请帮我写一个在 24 世纪的银河系发生的科幻故事，主要角色是进取号的传奇舰长 Picard，他们正在调查罗慕伦中立区的时候意外找到三个来自古地球的冬眠者，而罗穆伦人意外地与他们接触...

- **明确任务和要求，避免模糊不清的提问**  
    例：帮我写一篇 300 字的智能手表产品简介（响应格式），重点突出功能和使用场景，面向技术小白。
...
```bash
# 1. 依赖库（使用 pip 或者 uv pip 或者其它）
pip install openai-agents
```

```bash
# 2. 设置 API 密钥，没有可以不用 长期使用建议写入 /etc/profile (for Linux)
...
### 5.3 Agent Skill（智能体技能）

#### 5.3.1 什么是Agent Skills？
...
| 原则           | 说明                                 | 避雷点                   |
| -------------- | ------------------------------------ | ------------------------ |
| **单一职责**   | 一个Skill专注解决一类问题            | "全能"：什么都做但都不精 |
| **清晰触发**   | description明确说明"何时使用"        | 模糊描述导致模型误调用   |
| **结构化指令** | Markdown分层：快速开始→进阶→故障排除 | 大段文字无重点           |
| **可运行示例** | 提供完整代码+预期输出                | 只有理论描述             |
...
参考 Anthropic 博客：<https://www.creolestudios.com/anthropic-harness-design-for-reliable-ai-agents/>

#### 5.4.2 为什么需要Harness？

尽管 AI Agent 展现出强大的任务处理潜力，但其自身存在一些固有的局限性。因此，一个专门的管理与评估框架（Harness）至关重要。此处，可以参考上一节博客引文开头：
...
- LangChain 仅通过修改约束框架（而非模型），就显著提升了智能体性能
- OpenAI 通过结构化的约束框架设计，实现了如 Codex 这样的生产级系统

关键结论很简单：

**如果你的智能体运行不佳，优化模型往往是错误的方向。首先，修复你的约束框架。**
...
2026 年 3 月 31 日，Claude Code 新版本的 npm 包构建时意外包含了 source map 文件（cli.js.map），导致约 51.2 万行 TypeScript 源码被重建并公开。截至目前（2026.04.05），在 GitHub 上可能有数以百计的副本存在。这其中最有价值的部分是其 **Harness** 实践部分，即其约束框架的具体技术实现。此处我们不打算放置任何源代码以规避所有的合规性问题。但有兴趣的读者可以参考一个合规的开源项目（很大程度上参考了 Claude Code，并使用 Python 与 Rust）：<https://claw-code.codes>
...
- **何时引入哪一层：** 早期原型优先封装单一 Skill；当出现多工具/资料來源整合时引入 MCP；遇到多步骤/质量需求时，优先设计 Harness；Agent 用于协调业务逻辑。
- **三步走建议：**
  - 定义、实现1-2个可运行 Skill
...
上一章节讲述的概念是不少 AI 应用的框架，这一章节我们考察一些已经集成大部分套件，从而可以直接使用的 AI 应用。
...
另外，这股热潮吸引了很多非技术用户，大部分部署者没有多少计算机网络安全意识，将其服务端口直接暴露在公网并且不加密或者弱加密，很快就被夺取了主机权限。

**一些通用的提示与建议：**

- 不要把网关的服务端口（Port **18789**）暴露在公网，尤其是暴露在 **IPv4** 公网，全世界的 IPv4 地址只有四十亿个左右，每天每个地址都可能被扫描几遍，因此最好只在局域网尝鲜使用。若确有访问需求，应该使用专门的工具代理你的流量，比如 **Cloudflare Tunnel**；如果你的数据太过于敏感，则放弃远程访问。
...
| **Trae（字节）**        | 国内免费       | 集成 Claude 3.7/GPT-4o，中文优化  | [trae.cn](https://trae.cn)                                     |
