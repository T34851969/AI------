...
- 本文的内容分配侧重于介绍 AIGC 在编码中的运用
- 本文使用包括 Qwen-3.7、GPT-5.5、DeepSeek-V4 在内的大语言模型进行辅助写作（包括信息搜集、代码示例等）与排版。所有内容都经过了人工校对，并努力遵循一些通用的 AI 辅助写作原则
- 本文介绍的一些概念到目前（2026.06.20）依然不存在准确的官方译名，请以其英文为基准
- 本文的代码仅供示意参考，若读者想上手进一步的编码工作，请点击文档提供的官方 Wiki 跳转链接获取更多信息
- 本文可能存在事实疏忽、时效滞后或排版不当的问题，欢迎反馈
- 项目已经开源在 <https://github.com/T34851969/AI------>，欢迎提交 issue/PR
...
- [**AI工具使用文档 v1.2**](#ai工具使用文档-v12)
  - [**0 前言，以及目录**](#0-前言以及目录)
    ...
    - [目录](#目录)
  - [**1 关键概念**](#1-关键概念)
    - [理解以下概念（至少前面几个）至关重要，无论你打算或已使用过](#理解以下概念至少前面几个至关重要无论你打算或已使用过)
...
| **上下文**         | Context                             | ①对话上下文：历史记录保持连贯性；<br> ②提示上下文：引导模型生成预期回答   | Prompt 工程的核心，影响输出质量的关键                             |
...
| **模型上下文协议** | Model Context Protocol(MCP)         | 标准化大模型与外部数据源、工具和服务连接的开放协议                        | 统一资源/工具/提示词接入、安全隔离凭证管理                        |
| **检索增强生成**   | Retrieval-Augmented Generation(RAG) | 先从外部知识库或文档中检索相关内容，再让模型基于检索结果生成回答          | 先检索、后生成，减少幻觉，提升回答的准确性、时效性与可追溯性      |
...
| **约束框架**       | Harness                             | 在智能体系统中，负责评估、调度、编排和管理智能体技能使用的高级框架        | 规划技能调用、评估技能效果、优化任务执行流程，保证 Agent 稳定行为 |
...
本章先介绍氛围编程（Vibe Coding）的基本概念，再依次展开智能体、MCP、Agent Skill、Harness 以及 RAG 等技术。

### 5.1 Vibe Coding

**氛围编程**（Vibe Coding）大约在 2024 年的下半年逐渐成型，此时的 LLM 已经初具规模（GPT-4o 乃至 4.1 已可胜任简单工作）。真正出现是在 2025 年 2 月份，由 OpenAI 联合创始人 Andrej Karpathy 首次提出。简单讲，它就是“用 AI 写代码”的专名。
...
Vibe Coding 的核心可以描述为：**别读完 LLM 写的代码**（或至少有专人负责 Review）。否则你的工作将与传统编程无异，失去用 LLM 编程的意义。这种观点相当激进，也与当今生产趋势符合。
...
- **健全的编程与专业知识**
  ...
  如果你不认识基本的语法，以及行业内的通识、专识，那么 LLM 的代码于你而言就是黑箱，你甚至无法进行人工审查。至少两者之中必备一样，才能避免陷入 LLM 幻觉漩涡。
...
- **要懂代码**
  ...
  别读完 LLM 写的代码，但一定要做快速审查，尤其关注以下方面：
    ...
    3. 支付、认证、数据处理等敏感代码。这部分必须细致检查，以免风险

- **监督生成**

  如果你使用某些第三方中转站提供的 API，则很有可能被劫持返回的字段，向其中注入恶意代码（这种攻击成本低廉，因为由用户“同意”攻击），务必先做基础的审查（比如，拿另一个用于安全审计的 LLM 交叉验证）后再让其参与生产环境。

- **对初学者**

  初学者更应手动编码，否则谈不上对代码的理解，自然无从真正地使用 LLM 进行辅助。

LLM 从未降低参与生产编程的门槛，相反却提高了。因为大规模的 LLM 使用令大部分企业提出更严苛的绩效要求，在高压环境下想保持足够的编码质量，没有对语法、概念的熟悉，以及对业务框架的把握，是万分不行的。
...
- 对相关实现代码感兴趣的读者，可以参考一个用于教学示例的代码库：<https://github.com/MarkTechStation/VideoCod>
- 如果你希望快速部署一个 RAG 知识库，直接使用：<https://github.com/HKUDS/LightRAG>
...
失败处理：若 `fetch_data` 返回缺失关键指标，Harness 会触发重试/降级路径（例如用近似替代或标注数据缺口，并请求人工介入）。
...
通过将 AI 引入网关，OpenClaw 能够方便完成上述拓展功能。

#### 6.1.2 部署事项

相当完善的教程已经在 <https://docs.openclaw.ai/zh-CN/install> 提供，这里介绍安装前要览：

- 支持的平台：支持 Windows 系列、WSL2（官方推荐尽量在 **WSL2** 而不是通过 Windows PowerShell 部署）、MacOS，以及 Liunx。
- 如果你不从源码构建，只需要 Node 24 或者 22 。该项目是基于 Node.js 的，按照官网给出的教程恰当地安装，避免污染重要的环境变量。
...
- 整个部署最重要的前提：你已拥有对应 LLM 的 API Key。只有极少数 API 是免费的，比如硅基流动（SiliconFlow）提供一些 7B、8B 等小参数模型的免费 API，具体请访问：<https://siliconflow.cn/models> 。但是请注意，小模型存在更严重的安全审计及幻觉问题，仅适合验证部署。
...
- 选择一个现代的虚拟终端应用，PowerShell 本体对 UTF-8 字符、真彩色等显示不完善，可前往 Microsoft Store 下载 Windows Terminal。
...
- **原生安装（类Unix/Linux/WSL 可用，官方首推）**：`curl -fsSL https://claude.ai/install.sh | bash`

- **MacOS：** 虚拟终端下：`brew install --cask claude-code`
- **Linux：**
...
Claude Code 上手需要门槛，如果希望进一步学习，可以了解：<https://www.runoob.com/claude-code/claude-code-tutorial.html> 。
